...
public class DST {
    ...
    public static void getDSTDates(HashMap<Integer, Calendar> startDates, HashMap<Integer, Calendar> endDates) {
        ...
        for (String line: lines){ //For loop to iterate through each line.
            String[] temporary = line.split(","); //temporary array to store the splitting of the arraylist
            Calendar start = DateTools.parseDate(temporary[0]); //parsing the first value in the array to a calendar object
            Calendar end = DateTools.parseDate(temporary[1]);//parsing the second value in the array to a calendar object

            Integer currentYear = start.get(Calendar.YEAR); //Stores the year as an Integer for the hashmap

            startDates.put(currentYear, start); // Adds the starting date for the DST to the start hashmap
